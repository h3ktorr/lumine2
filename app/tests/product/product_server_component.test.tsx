...
import ProductPage from "@/app/product/[slug]/page"; // adjust path if needed
...
describe("Slug fetching", () => {
  ...
  it("should render Product_display when product is found", async () => {
    ...
    // const Page = await ProductPage({ params: Promise.resolve({ slug: "test-slug" }) });

    // render(Page);

    // expect(
    //   screen.getByText(mockProduct.name, { exact: false })
    // ).toBeInTheDocument();
  });
  ...
  //   await ProductPage({ params: Promise.resolve({ slug: "missing-slug" }) });
